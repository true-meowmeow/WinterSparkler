...
import core.ui.util.ComponentVisibility;
import core.main.config.BreakpointsProperties;
import core.main.config.CoreProperties;

import java.awt.*;

public class ThreeColumnLayout implements LayoutManager2, ComponentVisibility {
    ...
    public void addLayoutComponent(Component comp, Object constraints) {
        if (constraints instanceof ThreeColumnSlot role) {
            switch (role) {
                case COL1 -> c1 = comp;
                case COL2 -> c2 = comp;
                case COL3 -> c3 = comp;
    ...
    public void addLayoutComponent(String name, Component comp) {
        if ("col1".equals(name)) {
            addLayoutComponent(comp, ThreeColumnSlot.COL1);
        } else if ("col2".equals(name)) {
            addLayoutComponent(comp, ThreeColumnSlot.COL2);
        } else if ("col3".equals(name)) {
            addLayoutComponent(comp, ThreeColumnSlot.COL3);
        }
    }
    ...
    private void layoutThreeColumns(Insets in, int W, int H) {
        ...
        int w12 = LayoutCurves.eval(W, LayoutCurves.COL12);
